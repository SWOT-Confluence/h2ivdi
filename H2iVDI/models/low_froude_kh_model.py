...
class LowFroudeKhModel:
    def __init__(self, data, h0=1.0, h1=None, alpha0=10.0, alpha1=None):
        self._data = data
        self._k0 = k0
        if k1 is None:
    ...
    def calibrate_powerlaw(self, Q):
        ...
        data = self._data
        alphahat = np.zeros(data.x.size)
        betahat = np.zeros(data.x.size)
        h0hat = np.zeros(data.x.size)
        ...
        for ir in range(0, data.x.size):
            ...
            with model:
                ...
                # Priors for unknown parameters
                h0 = 0.07 + pm.Beta('h0p', alpha=1, beta=3.7673) * 15.68
                alpha = pm.Uniform('k', lower=5.0, upper=80.0)
                ...
                alphahat[ir] = np.median(np.ravel(trace["posterior"]["alpha"]))
                betahat[ir] = np.median(np.ravel(trace["posterior"]["beta"]))
                h0hat[ir] = np.median(np.ravel(0.07 + trace["posterior"]["h0p"] * 15.68))
                A0 = h0hat[ir] * np.min(Wrc)
